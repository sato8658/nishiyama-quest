...
export default function Home() {
  ...
  const resetAdventureProgress=()=>{setFeatureNotice(null);setTransport('');setCompanion('');setDuration('');setChosen([]);setCurrent(0);setDone([]);setPoints(0);setFoundPanda('');setFoundRedPandas([]);setGpsMessage('');setQuestToast('');setDemo(false);setRouteReady(false);setSavedRouteIds([]);setHasSavedAdventure(false);try{window.localStorage.removeItem(progressStorageKey);}catch{/* 保存不可でも画面上はリセット */}nav('transport');};
  ...
  return <main className="app-shell"><section className="phone-frame">
    ...
    {screen==='quest'&&active&&<div className="screen quest-screen">
      ...
      {active.type==='レッサーパンダ'&&selectedPandaRecord&&!reached&&<div className="arrival-guidance"><strong>現地に到着すると「見つけた！」を押せます</strong><span>{gps&&activeDistance!==null?`クエスト地点まで約${activeDistance}m`:'現在地を取得して到着を確認してください。'}</span></div>}
      ...
      <div className="quest-dots">{route.map((spot,index)=><span key={spot.id} className={done.includes(spot.id)?'done':index===current?'active':''}>{String(index+1).padStart(2,'0')}</span>)}</div>{questToast&&<div className="point-get">{questToast}</div>}
    </div>}
    ...
    {screen==='location'&&<div className="screen">
      ...
      <div className={`location-card ${gps?'location-card-compact':''}`}><div className="radar"><i/></div><strong>{demo?'QUEST POINTに到着！':reached?'QUEST POINTに到着！':gps?'現在地を取得済み':'現在地を取得してください'}</strong>{(!gps||demo)&&<p>{demo?'GPS到着判定のみスキップしています。':'現在地を取得すると距離を計算します。'}</p>}</div>
    ...
    {screen==='data'&&<div className="screen data-page">
      ...
      <p className="coordinate-note">一部クエスト地点は、西山公園内の施設周辺に登録されたオープンデータ位置情報を参照しています。</p>
