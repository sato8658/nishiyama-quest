...
export default function Home() {
  ...
  const visibleRedPandas=useMemo(()=>{
    const names=[...new Set([...openData.redPandas.map(panda=>panda.name),...redPandaPhotoRecords.map(photo=>photo.name)])];
    return names.map(name=>({name,photo:redPandaPhotoRecords.find(photo=>photo.name===name)}));
  },[openData]);
  ...
  const visitorTop=openData.visitorFlow.find(row=>row.dayType===(new Date().getDay()%6===0?'休日':'平日')&&row.rank===1);
  ...
  return <main className="app-shell"><section className="phone-frame">
    ...
    {screen==='top'&&<div className="hero screen">
      <div className="hero-visual"><img className="hero-bg" src="/assets/park-background.png" alt="緑豊かな公園のイメージ"/><div className="hero-overlay"/><div className="hero-title"><div className="eyebrow">SABAE · FUKUI</div><p className="quest-label">PARK ADVENTURE</p><h1>NISHIYAMA<br/><em>QUEST</em></h1><p className="catchcopy">今日の西山公園を、<br/>あなただけの<span className="accent">冒険</span>に。</p></div><img className="hero-panda" src="/assets/explorer-red-panda.png" alt="探検家レッサーパンダ"/></div>
      <div className="hero-actions">{hasSavedAdventure&&<button className="continue-button" onClick={()=>nav('quest')}>⚑ 冒険のつづきから <small>QUEST {Math.min(current+1,route.length)} / {route.length} · {points} POINT</small></button>}<button className="primary" onClick={requestNewAdventure}>{hasSavedAdventure?'新しい冒険を始める':'冒険をはじめる'} <span>›</span></button><p className="microcopy">実際のオープンデータから、あなた専用ルートをつくります</p></div>
    ...
    {screen==='route'&&<div className="screen">
      ...
      <section className="route-hero"><h2>{routeTitle(companion,chosen)}</h2><div className="route-summary"><span>所要時間 <b>約{route.reduce((sum,spot)=>sum+spot.minutes,0)}分</b></span><span>QUEST <b>{route.length}</b></span></div></section>
      ...
      <div className="route-list"><i>START</i>{route.map((spot,index)=><div className="spot-card" key={spot.id}><span>{String(index+1).padStart(2,'0')}</span><div><small>{spot.type} QUEST</small><strong>{spot.name}</strong>{spot.description&&<p>{spot.description}</p>}<em>約{spot.minutes}分 · 実データ</em></div></div>)}<i>GOAL</i></div>
    ...
    {screen==='complete'&&<div className="screen complete">
      <div className="complete-visual"><img className="complete-bg" src="/assets/park-background.png" alt=""/><img className="complete-panda" src="/assets/explorer-red-panda.png" alt="冒険完了を祝う探検家レッサーパンダ"/><div><p className="step-count">ALL QUESTS CLEARED</p><h2>冒険<br/>COMPLETE!</h2><p>西山公園の冒険をやりきりました。</p></div></div>
    ...
    {screen==='data'&&<div className="screen data-page">
      ...
      {[
        ['公共トイレ（福井県鯖江市）',openData.counts.publicToilets,'近くのトイレ・西山公園内ルート地点'],['駐車場（福井県鯖江市）',openData.counts.parking,'車利用時のスタート地点'],['バス停（福井県鯖江市）',openData.counts.busStops,'バス利用時のスタート地点'],['西山動物園のレッサーパンダ一覧',openData.counts.redPandas,'レッサーパンダクエスト'],['レッサーパンダ飼育個体情報',openData.counts.redPandaIndividuals,'個体プロフィール補助データ'],['令和7年度 西山公園入場者 市区町村別割合',openData.counts.visitorFlow,visitorTop?`${visitorTop.dayType}1位：${visitorTop.municipality}（${Math.round(visitorTop.ratio*100)}%）`:'来訪傾向表示'],
